...
class Model(object):
    ...
    def fit_batch(self, x_batch, y_batch, verbose=False):
        ...
        if hasattr(self, 'gradient_clip_value') and self.gradient_clip_value is not None: # Gradient clipping
            for layer in self._layers:
                for param_name in layer._parameter_updaters:
                    grad_name = param_name + '_grad'
                    if hasattr(layer, grad_name):
                        grad = getattr(layer, grad_name)
                        if grad is not None:
                            np.clip(grad, -self.gradient_clip_value, self.gradient_clip_value, out=grad)
    ...
    def fit(
        ...
        x_valid=None, y_valid=None, gradient_clip_value=None
    ):
    ...
    def get_metrics(self, y_gt, y_pred):
        loss = self._loss.value(y_gt, y_pred).item()
...
def convolve_pytorch(inputs, kernels, padding):
    ...
    with warnings.catch_warnings():
        ...
        warnings.filterwarnings(
            ...
            message=r"The given NumPy array is not write?able, and PyTorch does "
                    r"not support non-write?able tensors\..*"
        )
...
#     # Flip the kernel for convolution (PyTorch uses cross-correlation by default)
#     kernels_flipped = kernels[:, :, ::-1, ::-1]

#     # Ensure float32 dtype
#     inputs_torch = _torch.from_numpy(inputs).float()
#     kernels_torch = _torch.from_numpy(kernels_flipped).float()

#     # Ensure padding is int or tuple of ints
#     if isinstance(padding, (np.integer, float)):
#         padding = int(padding)
...
#                     r"not support non-write?able tensors\..*"
#         )
#         # Выполняем свёртку на CPU
#         result = _torch.nn.functional.conv2d(
#             inputs_torch,
#             kernels_torch,
#             padding=padding
#         )
